...
<ion-content :fullscreen="true">
	...
	<ion-refresher slot="fixed" :pull-factor="0.5" :pull-min="100" :pull-max="200" @ionRefresh="handleRefresh($event)"> <!-- eslint-disable-line vue/no-deprecated-slot-attribute -->
    ...
    <ion-item-sliding v-for="todo in activeTodos" :key="todo.id" :ref="(el) => setItemRef(el, todo.id!)">
      <ion-item-options side="start" @ionSwipe="handleDelete(todo)">
        <ion-item-option color="danger" expandable @click="handleDelete(todo)"> 
          <ion-icon slot="icon-only" :icon="trash" size="large"></ion-icon> <!-- eslint-disable-line vue/no-deprecated-slot-attribute -->
        </ion-item-option>
      </ion-item-options>
<!-- bagian refresher -->
...
<!-- active todos -->
      ...
      <ion-item-options side="end" @ionSwipe="handleStatus(todo)">
        <ion-item-option @click="handleEdit(todo)">
          <ion-icon slot="icon-only" :icon="create" size="large"></ion-icon> <!-- eslint-disable-line vue/no-deprecated-slot-attribute -->
        </ion-item-option>
        <ion-item-option color="success" expandable @click="handleStatus(todo)">
          <ion-icon slot="icon-only" :icon="checkmarkCircle" color="light" size="large"></ion-icon> <!-- eslint-disable-line vue/no-deprecated-slot-attribute -->
        </ion-item-option>
      </ion-item-options>
    </ion-item-sliding>
...
<ion-item class="accordion-container">
  <ion-accordion-group>
    <ion-accordion value="first">
      <ion-item slot="header" color="light"> <!-- eslint-disable-line vue/no-deprecated-slot-attribute -->
        <ion-label class="ion-text-center">Completed</ion-label>
      </ion-item>
      <div slot="content" class="scrollable-container"> <!-- eslint-disable-line vue/no-deprecated-slot-attribute -->
        <ion-list>
  ...
  <ion-fab vertical="bottom" horizontal="end" slot="fixed"> <!-- eslint-disable-line vue/no-deprecated-slot-attribute -->
